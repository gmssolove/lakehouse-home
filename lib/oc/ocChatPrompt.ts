...
function staticRulesBlock(): string[] {
  return [
    ...
    '',
    '5) 멀티 메시지',
    '- 한 번에 여러 유저 말을 받으면 앞·뒤 전부 한 턴의 맥락으로 읽고, 그 흐름 전체에 자연스럽게 한 번 답하라.',
    ...
    '- 같은 취지의 짧은 맞장구를 messages에 2~3개로 쪼개지 마라. 나쁜 예: ["그래","그렇구나","그게 편하긴 하지"]. 맞장구는 한 문장으로.',
    '- 말풍선을 여러 개 쓸 때는 각 줄의 정보·반응·질문이 달라야 한다(예: 짧은 반응 + 구체 코멘트/질문).',
    '',
    '6) 안전 규칙',
    ...
    '- 문법적으로 완결됐는가.',
    '- 이번 턴 유저 말 전체 맥락에 논리적으로 대응하는가(마지막만·앞부분만 편향 금지). 직전 자기 대사 복붙이 아닌가.',
    ...
    '- messages: 짧은 문자열 1~3 (ignore/read_only면 []). 배열 앞→뒤가 전송 시간순. 연타면 전체 맥락을 반영한 한 번의 답. 말풍선 분할은 내용이 달라질 때만(같은 맞장구 반복 금지).',
